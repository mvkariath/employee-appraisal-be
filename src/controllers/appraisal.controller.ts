...
class AppraisalController {
    ...
    constructor(private appraisalService: AppraisalService, router: Router) {
        router.post("/", checkRole([EmployeeRole.HR, EmployeeRole.LEAD]), this.createAppraisals.bind(this));
        router.get("/", this.getAllAppraisals.bind(this));
        router.get("/:id", this.getAppraisalById.bind(this));
        router.put("/:id", checkRole([EmployeeRole.HR, EmployeeRole.LEAD]),auditLogMiddleware, this.updateAppraisal.bind(this));
        router.delete("/:id", checkRole([EmployeeRole.HR]), this.deleteAppraisal.bind(this));
    }

    async createAppraisals(req: Request, res: Response, next: NextFunction) {
        try {
            const { employeeIds, cycle, content, remarks_by, current_status, submitted_at, closed_at } = req.body;
    ...
    async updateAppraisal(req: Request, res: Response, next: NextFunction) {
        try {
            const id = Number(req.params.id);
            if (isNaN(id)) throw new HttpException(400, "Invalid appraisal ID");
            const updated = await this.appraisalService.updateAppraisalById(id, req.body);
