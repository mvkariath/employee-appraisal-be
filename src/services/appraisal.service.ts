...
async createAppraisals(data: {
  ...
  return createdAppraisals;
    ...
    async updateAppraisalById(id: number, data: Partial<Appraisal>): Promise<void> {
    ...
    async deleteAppraisalById(id: number): Promise<void> {
        ...
        if (existingAppraisal) {
            ...
            this.logger.info(`removeAppraisalById - SUCCESS: Removed appraisal with ID ${id}`);
        } else {
            this.logger.error(`removeAppraisalById - NOT FOUND: appraisal with ID ${id}`);
